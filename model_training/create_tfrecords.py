...
LABEL_PATH = "tools_dataset_label_map.pbtxt"

num_shards = 10
output_record = os.path.join(RECORDS_DIR, "train", "tools_dataset.record")
output_test = os.path.join(RECORDS_DIR, "eval", "tools_dataset.record")
...
def main(_):
	with contextlib2.ExitStack() as tf_record_close_stack:
		if not os.path.exists(os.path.dirname(output_record)):
			try:
				os.makedirs(os.path.dirname(output_record))
			except OSError as exc:
				if exc.errno != errno.EEXIST:
					raise
		if not os.path.exists(os.path.dirname(output_test)):
			try:
				os.makedirs(os.path.dirname(output_test))
			except OSError as exc:
				if exc.errno != errno.EEXIST:
					raise
		output_tfrecords = tf_record_creation_util.open_sharded_output_tfrecords(tf_record_close_stack, output_record, num_shards)
		output_tftest = tf_record_creation_util.open_sharded_output_tfrecords(tf_record_close_stack, output_test, num_shards)
		...
		for wnid in natsorted(os.listdir(IMAGES_DIR)):
			...
			display_name = subprocess.Popen(["curl", "-s", "http://www.image-net.org/api/text/wordnet.synset.getwords?wnid=" + wnid],stdout=subprocess.PIPE).communicate()[0]
			display_name = ", ".join(display_name.split("\n")[:-1])
			with open(LABEL_PATH, 'a') as pbtxt:
				pbtxt.write("item {\n")
				pbtxt.write("  name: \"" + wnid + "\"\n")
				pbtxt.write("  id: " + str(id) + "\n")
				pbtxt.write("  display_name: \"" + display_name + "\"\n")
				pbtxt.write("}\n")
			...
			for image in natsorted(os.listdir(os.path.join(IMAGES_DIR, wnid))):
				...
				if os.path.isfile(image_xml):
					...
					output_tftest[output_shard_index].write(tf_example.SerializeToString())
